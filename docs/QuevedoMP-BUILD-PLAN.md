...
| P7 | **(NEW, from the P2 A/B) CPU self-collision is the dominant cost on many-link robots** — batch it in parallel (the OptiX backend already overlaps it with the GPU trace, but it runs serially across configs) and/or prune the checked pair set. On the benchmark cell: self+env 70 µs/config vs env-only 10 µs — self costs 4× the whole environment query, capping the hybrid at FCL speed. Expected ≥5× on the studio benchmark once configs are checked across cores. | A/B on `benchmark.qmps` scene, 2048-config batch: FCL 35→150 ms and OptiX 20→140 ms when self-collision turns on. |
