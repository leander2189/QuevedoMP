...
| P8 | **(NEW, from P7a) Recalibrate the Auto hybrid crossover**: with parallel CPU batches, FCL now beats the GPU on the mid-poly benchmark cell (0.72 s vs 0.84 s) — "fat batch ⇒ GPU" no longer holds independent of scene complexity. The Task 2b.2 latency profile should set the threshold per scene (e.g. by environment triangle count), and re-check on the ≥1M-triangle Phase B fixtures where the GPU should still win outright. | `benchmark.qmps` post-P7a: Auto routed fat batches to OptiX and lost ~15% to parallel FCL. |
