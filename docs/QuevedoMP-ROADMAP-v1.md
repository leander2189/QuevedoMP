...
| R9 | Dresskit (supply-chain) analysis — **post-processing only**, never a planning constraint: a clamped elastic-rod solver (node coords + inextensibility, SQP) over an already-planned trajectory, reporting clearance / min-bend-radius / slack / fatigue, `min_feasible_length` study and swept envelope, drawn in a studio Dresskit mode. Pure Python+numpy, **zero C++ changes**. | M | — (design: `docs/QuevedoMP-R9-DESIGN.md`, 2026-08-01) |
| R10 | Planning-throughput reorder — **self-collision first, GPU environment second**. Measured on the real cell: making environment collision free is `1.01×`, making self-collision free is `3.51×` (self is 91% of every query and runs on the CPU in *both* backends). Phases: integer self-ACM → self-pair reachability pruning → convex decomposition → P7b → the OptiX pipeline work (cull default, streams, joint-angle upload, GPU FK). Does **not** address solvability — R8 stays ahead of it. | M–L | — (design: `docs/QuevedoMP-R10-DESIGN.md`, 2026-08-25) |
